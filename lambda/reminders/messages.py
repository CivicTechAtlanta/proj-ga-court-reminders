...
def reminder_id(label: str, hearing, to: str) -> str:
    """A stable id for "this reminder, for this hearing, to this number".
    ...
    """
    when = _event_stamp(hearing.event_datetime)
    return f"{label}:{hearing.case_party_id}:{when}:{_number_tag(to)}"


def _event_stamp(event_datetime) -> str:
    if isinstance(event_datetime, datetime):
        return event_datetime.strftime("%Y%m%dT%H%M")
    # A driver that hands back a string still has to produce a stable id.
    return str(event_datetime).replace(" ", "T")
