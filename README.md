...
Every script below is run from this directory.

### Step 4: Create your .env

The scripts read their settings from `.env`, which is gitignored and does not
...
The copy is enough to get everything running. It already points at Floci, the
local AWS emulator, which is what lets the scripts work without an AWS
account.

**Sending real text messages is the one thing it does not cover.** That needs
three values from your TrueDialog credentials email and the TrueDialog portal,
which you can add whenever you need them:
...
See [Credentials and what never to
...
`./script/setup` checks the tools, installs the Python dependencies, starts
Floci, bootstraps it for CDK, and deploys both CDK stacks with dummy
credentials. The deploy creates a Postgres database inside Floci and seeds it
with the court fixtures, then runs the seven-day hearing query against it. The
first run takes a minute or two, mostly building the Lambda bundles. It has
worked when the output ends with something like:
...
Without TrueDialog credentials it also prints a note saying no text can be
sent yet. That is expected, and nothing else is affected.
